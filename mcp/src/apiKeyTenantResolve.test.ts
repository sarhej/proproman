...
const tymioFetch = vi.fn();

vi.mock("./api.js", () => ({
  tymioFetch: (...a: unknown[]) => tymioFetch(...a),
...
describe("resolveTenantIdForWorkspaceSlug", () => {
  ...
  it("returns tenant id when slug matches ACTIVE membership (case-insensitive)", async () => {
    ...
    expect(tymioFetch).toHaveBeenCalledWith("/api/me/tenants");
  });
