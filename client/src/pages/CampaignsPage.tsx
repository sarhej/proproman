...
function InlineAdd({ placeholder, onAdd }: { placeholder: string; onAdd: (val: string) => Promise<void> }) {
  ...
  return (
    <span className="inline-flex items-center gap-1">
      ...
      />
      <button type="button" className="text-[10px] text-slate-400 hover:text-slate-600" onClick={() => { setValue(""); setAdding(false); }}>{t("common.cancel")}</button>
    </span>
  );
}
...
function DeleteBtn({ label, onDelete }: { label: string; onDelete: () => Promise<void> }) {
  const { t } = useTranslation();
  return (
    <button type="button" className="ml-1 inline-flex opacity-0 group-hover/row:opacity-100 text-slate-400 hover:text-red-500 transition-opacity" title={t("productTree.deleteConfirm", { name: label })}
      onClick={async (e) => { e.stopPropagation(); if (window.confirm(t("productTree.deleteConfirm", { name: label }))) await onDelete(); }}
    >
      <Trash2 size={12} />
    </button>
  );
...
function EditableTitle({ title, onSave, className }: { title: string; onSave: (v: string) => Promise<void>; className?: string }) {
  ...
  if (!editing) return <span className={`cursor-text ${className ?? ""}`} onDoubleClick={() => setEditing(true)} title={t("common.doubleClickToEdit")}>{title}</span>;
  return (
  ...
  return (
    <tr className="group/row border-t border-slate-100 text-xs hover:bg-slate-50">
      ...
      <td className="px-2 text-center">
        {isAdmin ? (
          <select className="rounded border border-slate-200 px-1 py-0.5 text-[10px]" value={asset.type}
            ...
            {ASSET_TYPES.map((type) => <option key={type} value={type}>{t(`assetType.${type}`)}</option>)}
          </select>
        ) : <span className="rounded bg-slate-100 px-1.5 py-0.5 text-[10px]">{t(`assetType.${asset.type}`)}</span>}
      </td>
      ...
      <td className="px-2 text-center">
        ...
        ) : <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${statusColor(asset.status)}`}>{t(`assetStatus.${asset.status}`)}</span>}
      </td>
    </tr>
  );
}
    ...
    <>
      <tr className="group/row border-t-2 border-slate-300 bg-slate-50 text-sm hover:bg-slate-100">
        <td className="py-2.5 pl-2 pr-2">
          ...
          <span className="ml-2 text-xs font-normal text-slate-500">{t("campaigns.assets", { count: campaign.assets.length })}</span>
        ...
        <td className="px-2 text-center">
          {isAdmin ? (
            <select className="rounded border border-slate-200 px-1 py-0.5 text-[10px]" value={campaign.type}
              ...
              {CAMPAIGN_TYPES.map((type) => <option key={type} value={type}>{t(`campaignType.${type}`)}</option>)}
            </select>
          ) : <span className="rounded bg-slate-100 px-1.5 py-0.5 text-[10px]">{t(`campaignType.${campaign.type}`)}</span>}
        </td>
        ...
        <td className="px-2 text-center">
          {isAdmin ? (
            <select className="rounded border border-slate-200 px-1 py-0.5 text-[10px]" value={campaign.status}
              ...
              {CAMPAIGN_STATUSES.map((s) => <option key={s} value={s}>{t(`campaignStatus.${s}`)}</option>)}
            </select>
      ...
      {open ? (
        <>
          ...
          <tr className="border-t border-slate-100 bg-white text-xs">
            <td colSpan={5} className="px-8 py-2">
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-1">
                <span className="text-[10px] font-semibold uppercase text-slate-400 mr-1">{t("campaigns.linked")}</span>
                  ...
                  addingLink ? (
                    <span className="inline-flex items-center gap-1 text-[10px]">
                      <select className="rounded border px-1 py-0.5" value={linkInit} onChange={(e) => setLinkInit(e.target.value)}>
                        <option value="">{t("campaigns.selectInitiative")}</option>
                      ...
                      <button type="button" className="text-slate-400 hover:text-slate-600" onClick={() => setAddingLink(false)}>{t("common.cancel")}</button>
                    </span>
                  ) : (
          ...
          <tr className="border-t border-slate-200 bg-slate-50 text-[10px] font-semibold uppercase text-slate-400">
            <th className="pl-12 pr-2 py-1">{t("campaigns.asset")}</th>
        ...
        <table className="w-full min-w-[800px] text-left">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-100 text-xs font-semibold uppercase text-slate-500">
              <th className="px-2 py-2">{t("campaigns.campaign")}</th>
