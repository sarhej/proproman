...
**MCP CLI / OAuth (canonical):** [`mcp/TYMIO_MCP_CLI_AGENT_GUIDANCE.md`](../mcp/TYMIO_MCP_CLI_AGENT_GUIDANCE.md) — tell users to use OAuth or `tymio-mcp login`; there is **no** MCP API key in Tymio user Settings. Also: `GET /api/mcp/agent-context` → `tymioMcpCliAgentGuidanceMarkdown`.
