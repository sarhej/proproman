...
- **Backlog / data (historical `tymio_*` prefix):** health, meta, initiatives, features, requirements, domains, products, accounts, partners, demands, campaigns, timeline, assignments, stakeholders, etc. Full list: `server/src/mcp/tools.ts`; **canonical tool names for ontology** live in `server/src/mcp/registeredMcpToolNames.ts`. Stdio package subset: `mcp/src/apiKeyStdio.ts`.
...
2. Optionally, on **full MCP**, call **`tymio_get_workspace_atlas`** and use **`tymio_search_workspace_objects`** / **`tymio_get_workspace_object`** to scan a large backlog with fewer round-trips than repeated `tymio_list_*` calls. If you see **`not_built`**, an editor can run **`tymio_rebuild_workspace_atlas`**.
3. **`tymio_meta`** (or `GET /api/meta`) for domains, products, users, accounts, partners, personas, revenue streams.
4. **`tymio_list_initiatives`** with filters if available, or search in UI Product Explorer.
5. Open **`tymio_get_initiative`**, **`tymio_list_features`** / feature detail, **`tymio_list_requirements`** for the initiative you care about.
