...
- Create or use a **workspace** (tenant) as an **organization (B2B)**;
- Use Tymio as an **individual or small team (B2Vibecoders)**;
- Authenticate via **Google, Microsoft**, email magic links, or other methods we support;
- Use **APIs, MCP**, or other automation features tied to your account.

We process personal data in line with the **GDPR** (EU/EEA/UK, as applicable) and applicable **ePrivacy** rules for cookies and similar technologies.

---

## 2. Roles: controller vs processor

### 2.1 Provider as controller

We act as **controller** for personal data we decide **how and why** to process, for example:

- Account and billing records;
- Service operations, security, fraud prevention;
- Product analytics **about use of the Service** (in line with this Policy);
...
If you are an **individual** workspace owner without a separate corporate controller relationship, we are generally **controller** for **account** data and **processor-like** obligations still apply to **your workspace content** as described in your agreement with us; in practice, **you** decide what you store in the product.

---

## 3. Categories of personal data we process

Depending on how you use Tymio, we may process:

| Category | Examples |
|----------|----------|
| **Identity & account** | Name, email, profile image from OAuth provider, internal user ID, workspace memberships, roles (platform and tenant). |
| **Authentication & security** | Session tokens, device/browser signals, IP address, timestamps, audit logs, API key metadata (if enabled). |
| **Workspace / product content** | Text and structured records **you** enter (may include names, emails, job titles of colleagues or partners). |
| **Usage & diagnostics** | Logs, error reports, feature usage **in aggregate or pseudonymous** form where possible. |
| **Support** | Messages you send us, attachments, ticket metadata. |
| **Billing** (if applicable) | Billing contact, address, VAT ID, payment references; **card data** is handled by **payment providers** where used. |
| **Marketing** (if opted in or permitted) | Newsletter preferences, campaign engagement. |

We do **not** require you to submit **special categories** of data (GDPR Art. 9) in Tymio. **Do not** use the Service to store health, biometric ID data, or similar **unless** we have agreed **in writing** with appropriate safeguards.
...
| Purpose | Legal basis |
|---------|-------------|
| **Provide the Service** (hosting, sync, search, notifications) | **Contract** (Art. 6(1)(b)) |
| **Security, abuse prevention, integrity** | **Legitimate interests** (Art. 6(1)(f)) — balancing test documented internally |
| **Compliance with law** (tax, court orders) | **Legal obligation** (Art. 6(1)(c)) |
| **Product improvement (non-essential analytics)** | **Consent** or **legitimate interests** where allowed; we minimize identifiers |
| **Marketing emails** | **Consent** (Art. 6(1)(a)) or **soft opt-in** only where strictly permitted |
| **Processor processing on Customer instructions** | **Contract** with Customer (Art. 6(1)(b)) / **legitimate instructions** |

Where we rely on **legitimate interests**, you may **object** under Art. 21; see §10.

---

## 5. Sources

- **You** (forms, settings, uploads);
- **Your organization** (invites, SSO attributes if configured);
- **OAuth providers** (Google, Microsoft) — limited to what you authorize;
- **Automated systems** (logs, security tooling).
...
If personal data is processed **outside the EEA/UK**, we implement **appropriate safeguards**, such as:
...
- **Restrict** processing;
- **Data portability** (for contract/ consent-based processing where technically feasible);
...
We will post updates here and revise the **“Last updated”** date. **Material** changes will be communicated as required by law (e.g. email or in-app).
