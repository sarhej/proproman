...
**Rule for agents:** Do **not** tell users to copy an **MCP API key** from Tymio **Settings** — it does not exist. Use **OAuth** (remote **`/mcp`** or **`/t/<workspace-slug>/mcp`** URL in the IDE, or `tymio-mcp login` for the stdio package).
