...
export const vcsOauthGitlabRouter = Router();

function redirectBase(): string {
  const base = (env.API_PUBLIC_URL ?? "").replace(/\/$/, "") || `http://127.0.0.1:${env.PORT}`;
  return `${base}/api/vcs/oauth/gitlab/callback`;
}

function gitlabHost(): string {
  const u = env.VCS_GITLAB_BASE_URL?.replace(/\/$/, "") || "https://gitlab.com";
  return u;
}

vcsOauthGitlabRouter.get("/start", requireAuth, requireTenant, async (req, res) => {
  ...
  if (!env.VCS_GITLAB_CLIENT_ID || !env.VCS_GITLAB_CLIENT_SECRET) {
    res.status(503).json({ error: "GitLab VCS OAuth is not configured." });
    return;
  }
  ...
  const url = `${gitlabHost()}/oauth/authorize?client_id=${encodeURIComponent(env.VCS_GITLAB_CLIENT_ID)}&response_type=code&state=${encodeURIComponent(state)}&redirect_uri=${redirectUri}&scope=${encodeURIComponent("read_api api read_repository")}`;
  res.redirect(url);
});

vcsOauthGitlabRouter.get("/callback", async (req, res) => {
  ...
  const body = new URLSearchParams({
    client_id: env.VCS_GITLAB_CLIENT_ID,
    client_secret: env.VCS_GITLAB_CLIENT_SECRET,
    code,
    grant_type: "authorization_code",
    redirect_uri: redirectBase()
  });
  const tokenRes = await fetch(`${gitlabHost()}/oauth/token`, {
    method: "POST",
  ...
  if (!tokenJson.access_token) {
    res.status(400).send("GitLab token exchange failed");
    return;
  }
