...
description: >-
  ...
  Tymio, tymio.app, DRD tools, drd_* or tymio_* MCP tools, workspace hub,
...
- **`https://tymio.app/t/<workspace-slug>/mcp`** — **Workspace MCP** (full **`drd_*`** + workspace **`tymio_*`**). Same OAuth flow; slug is in the path.
...
| **Stdio `@tymio/mcp-server`** | For full tools, set **`TYMIO_MCP_URL`** / **`tymio-mcp login`** to **`…/t/<slug>/mcp`**. Root **`…/mcp`** only proxies the two discovery tools unless the user changes URL. Ensure **`DRD_API_KEY` / `API_KEY` are unset** for OAuth proxy mode. |
...
Be explicit, not vague: e.g. “Tymio MCP is not authenticated in this session. Please complete sign-in for the Tymio MCP server (or run `tymio-mcp login` for stdio). After that, ask me to retry starting with `drd_health`.” Do **not** imply the hub was updated when you never got a 200 from an authenticated call.
...
1. If you only see **two** tools and no **`drd_*`**, say clearly: user is on **root `/mcp`** — they must add **`…/t/<slug>/mcp`** for the target org.
...
- **Expecting `drd_meta` on root `/mcp`:** Wrong endpoint — use **`…/t/<slug>/mcp`**.
...
2. **OAuth gate:** Follow the section **OAuth and session — do this before any hub tool** at the top of this skill — `mcp_auth` or IDE sign-in / `tymio-mcp login`, then **`drd_health` or `tymio_get_agent_brief`** to verify.
3. **Confirm you are actually connected.** MCP tools (`drd_*`, `tymio_*`) exist only if the runtime has a working Tymio MCP config. If tools are missing or calls fail with auth/connection errors, **do not** claim data changed — tell the user to fix MCP or use REST with a key.
...
5. `drd_meta` or `GET /meta` — resolve `domainId`, `productId`, etc., after auth (tenant-scoped — same IDs are invalid in another workspace).
6. List/update work: `drd_list_initiatives`, `drd_get_initiative`, `drd_list_features`, `drd_list_requirements`, and matching `drd_update_*` or REST PATCH.
